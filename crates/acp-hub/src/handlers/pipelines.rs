use acp_protocol::{
    ArtifactCreateRequest, ArtifactRecord, ContextCompressionCreateRequest,
    ContextCompressionRecord, PipelineCreateRequest, PipelineEventCreateRequest,
    PipelineEventRecord, PipelineRecord, PipelineStatus, PipelineStatusUpdateRequest, RoleSlot,
    SchedulerDecision, SchedulerDecisionCreateRequest, SemanticMemoryCreateRequest,
    SemanticMemoryRecord, SemanticSearchResponse, SlotStatus, SlotUpdateRequest,
    StepMetricCreateRequest, StepMetricsRecord, WorkflowConfig, WorkingContext,
...
use crate::{
    authorize,
    db::{
        get_pipeline_by_id, get_pipeline_event_by_id, get_slot_by_role, insert_context_compression,
        insert_pipeline_event, insert_scheduler_decision, insert_semantic_memory,
        insert_step_metric, pipeline_context_compressions, pipeline_scheduler_decisions,
        pipeline_semantic_memory, row_to_artifact, row_to_pipeline, row_to_pipeline_event,
        row_to_slot, row_to_working_context,
    },
    ApiError, HubState,
};
...
pub(crate) async fn list_pipeline_events(
    ...
    authorize(&state, &headers)?;
    let rows = sqlx::query("SELECT id, pipeline_id, agent_id, event_type, payload, correlation_id, causation_id, created_at FROM pipeline_events WHERE pipeline_id = ?1 AND id > ?2 ORDER BY id ASC")
        .bind(id.to_string())
...
pub(crate) async fn list_scheduler_decisions(
    State(state): State<HubState>,
    headers: HeaderMap,
    Path(id): Path<Uuid>,
) -> Result<Json<Vec<SchedulerDecision>>, ApiError> {
    authorize(&state, &headers)?;
    pipeline_scheduler_decisions(state.pool(), id)
        .await
        .map(Json)
}

pub(crate) async fn create_scheduler_decision(
    State(state): State<HubState>,
    headers: HeaderMap,
    Path(id): Path<Uuid>,
    Json(mut req): Json<SchedulerDecisionCreateRequest>,
) -> Result<Json<SchedulerDecision>, ApiError> {
    authorize(&state, &headers)?;
    req.pipeline_id = id;
    insert_scheduler_decision(state.pool(), req).await.map(Json)
}

pub(crate) async fn list_context_compressions(
    State(state): State<HubState>,
    headers: HeaderMap,
    Path(id): Path<Uuid>,
) -> Result<Json<Vec<ContextCompressionRecord>>, ApiError> {
    authorize(&state, &headers)?;
    pipeline_context_compressions(state.pool(), id)
        .await
        .map(Json)
}

pub(crate) async fn create_context_compression(
    State(state): State<HubState>,
    headers: HeaderMap,
    Path(id): Path<Uuid>,
    Json(mut req): Json<ContextCompressionCreateRequest>,
) -> Result<Json<ContextCompressionRecord>, ApiError> {
    authorize(&state, &headers)?;
    req.pipeline_id = id;
    insert_context_compression(state.pool(), req)
        .await
        .map(Json)
}

pub(crate) async fn list_semantic_memory(
    State(state): State<HubState>,
    headers: HeaderMap,
    Path(id): Path<Uuid>,
) -> Result<Json<Vec<SemanticMemoryRecord>>, ApiError> {
    authorize(&state, &headers)?;
    pipeline_semantic_memory(state.pool(), id).await.map(Json)
}

pub(crate) async fn create_semantic_memory(
    State(state): State<HubState>,
    headers: HeaderMap,
    Path(id): Path<Uuid>,
    Json(mut req): Json<SemanticMemoryCreateRequest>,
) -> Result<Json<SemanticMemoryRecord>, ApiError> {
    authorize(&state, &headers)?;
    req.pipeline_id = id;
    insert_semantic_memory(state.pool(), req).await.map(Json)
}
...
pub(crate) async fn search_semantic_memory(
    State(state): State<HubState>,
    headers: HeaderMap,
    Path(id): Path<Uuid>,
    ...
    authorize(&state, &headers)?;
    let mut records = pipeline_semantic_memory(state.pool(), id).await?;
    records.sort_by(|a, b| {
        semantic_score(&b.content, &query.q)
            .partial_cmp(&semantic_score(&a.content, &query.q))
...
fn semantic_score(content: &str, query: &str) -> f64 {
